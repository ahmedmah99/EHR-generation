...
public class HeartFailureGen {


    File file;
    FileWriter writer;
    public HeartFailureGen(File file, FileWriter fileWriter){
        this.file = file;
    ...
    public void start() throws IOException {
        ...
        List<List<String>> allRecords = new ArrayList<List<String>>(5173);
    ...
    private void healthyPeopleGen(List<List<String>> allRecords){
        ...
        float WBC = truncate(this.nextFloat(11500,13500));
        record.add(WBC+"");
        ...
        float Mono = truncate(this.nextFloat(7,10)); //percentage
        Mono = (Mono/100) * WBC;
        record.add(Mono+"");
        ...
        int sodium = this.nextInt(136,148);
        record.add(sodium+"");

        float Potassium = truncate(this.nextFloat(3.5,5.6));
        record.add(Potassium+"");

        int Chloride = this.nextInt(98,106);
        record.add(Chloride +"");

        int Bicarbonate = this.nextInt(23,30);
        ...
        int BNP =  this.nextInt(100,181);
        record.add(BNP+"");

        int EF = this.nextInt(10,41); //percentage
        record.add(EF+"");
        ...
        int Breathlessness = this.nextInt(3,6);
        record.add(Breathlessness+"");

        int Fatigue = this.nextInt(3,6);;
        record.add(Fatigue+"");

        int SwollenAnklesAndLegs = this.nextInt(3,6);
        ...
        int Cough = (_Uncommonsymptoms >= 8? this.nextInt(2,5) : this.nextInt(0,2));
        record.add(Cough+"");

        int RunnyNose = 0;
        record.add(RunnyNose+"");

        int Hunger = (_Uncommonsymptoms >= 8? this.nextInt(1,5) : this.nextInt(0,2));
        record.add(Hunger+"");
        ...
        int Confusion = (_Uncommonsymptoms >= 8? this.nextInt(2,5) : this.nextInt(0,2));
