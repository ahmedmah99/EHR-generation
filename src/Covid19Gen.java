...
public class Covid19Gen {


    File file;
    FileWriter writer;
    public Covid19Gen(File file,FileWriter fileWriter){
        this.file = file;
    ...
    public void CovidStart() throws IOException {
        ...
        for (int i  = 0; i < capacity; i++){
            covidPatientGenerator(allRecords);
    ...
    private void covidPatientGenerator(List<List<String>> allRecords){
        ...
        int CovidSeverence = nextInt(1,11);


        int WL = this.nextInt(3,6); //weight lose or poor appetite
        record.add(WL+"");

        int Breathlessness = (CovidSeverence >= 9? this.nextInt(4,6) : this.nextInt(1,4));
        ...
        int Tiredness = this.nextInt(3,6);
        ...
        int SoreThroat = (CovidSeverence >5? this.nextInt(4,6) : this.nextInt(1,4));
        record.add(SoreThroat+"");

        int HeadAche = (CovidSeverence >5? this.nextInt(4,6) : this.nextInt(1,4));
        record.add(HeadAche+"");

        int BodyAches = (CovidSeverence >5? this.nextInt(4,6) : this.nextInt(1,4));
        ...
        int Diarrhoea = (CovidSeverence >5? this.nextInt(4,6) : this.nextInt(1,4));
        ...
        int Rash = (CovidSeverence >5? this.nextInt(4,6) : this.nextInt(1,4));
        record.add(Rash+"");

        int Chills = this.nextInt(3,6);
        record.add(Chills+"");

        int StuffyNose = this.nextInt(0,2);
        record.add(StuffyNose+"");

        String Conjunctivitis = "1";
