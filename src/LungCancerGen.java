...
public class LungCancerGen {


    File file;
    FileWriter writer;
    public LungCancerGen(File file, FileWriter fileWriter){
        this.file = file;
    ...
    public void lungCancerStartFunc() throws IOException {
        ...
        for (int i  = 0; i < capacity; i++){
            lungCancerPatientGen(allRecords);
    ...
    private void lungCancerPatientGen(List<List<String>> allRecords){
        ...
        String Bronchoscopy = "1";
        record.add(Bronchoscopy);


        int WL = this.nextInt(2,6); //weight lose or poor appetite
        record.add(WL+"");

        int Breathlessness = this.nextInt(2,6);
        ...
        int Tiredness = this.nextInt(1,5);
        record.add(Tiredness+"");

        int Cough = this.nextInt(2,6);
        record.add(Cough+"");
